	...
	class WPCW_UnitFrontend {
		...
		public function __construct( $post ) {
			...
			if ( $this->parentData ) {
				...
				// Get quiz details
				$this->unitQuizDetails = WPCW_quizzes_getAssociatedQuizForUnit( $this->unitPost->ID, $this->currentUserID, true );
		...
		public function check_timers_doWeHaveAnActiveTimer_thatHasExpired() {
			...
			// Check if quiz has timer
			if ( 'no_timer' === $this->unitQuizDetails->quiz_timer_mode ){
				return;
			}
		...
		public function update_quizzes_requestQuizRetake() {
			...
			", $this->currentUserID, $this->unitPost->ID ) );
				...
				if( isset( $this->unitQuizProgress ) && is_object( $this->unitQuizProgress ) ) {
				...
				foreach ( $this->unitQuizDetails->questions as $question ) {
					...
					if ( $setting_showUserAnswer ) {
						...
						if ( ! is_null( $theirAnswer ) ) {
							...
							if ( 'yes' == WPCW_arrays_getValue( $theirAnswerDetails, 'provide_feedback_radio_quiz' ) ){
								if ( ! empty( WPCW_arrays_getValue( $theirAnswerDetails, 'provide_feedback_textarea_quiz' ) ) ){
									$html .= sprintf( '<div class="wpcw_fe_quiz_q_feedback"><b>%s:</b>&nbsp;&nbsp;<ul><li>%s</li></ul></div>', __( 'Instructor Feedback', 'wp-courseware' ), WPCW_arrays_getValue( $theirAnswerDetails, 'provide_feedback_textarea_quiz' ) );
								}
							}
		...
		public function render_completionBox_contentLockedDueToDripfeed() {
			...
			return apply_filters( 'wpcw_drip_lock_details', $lockDetails );
		}
		...
		public function render_completionBox_pending() {
			...
			return apply_filters( 'wpcw_front_completion_box_pending', $html );
		}
		...
		public function render_quizzes_handleQuizRendering( $showErrorsOnForm = true ) {
			...
			// #### 1 - Quiz Title - constant for all quizzes
			$html .= sprintf( '<div class="wpcw_fe_quiz_title"><h2>%s</h2></div>', $this->unitQuizDetails->quiz_title );

			// #### 2 - Pass mark - just needed for blocking quizes
			if ( 'quiz_block' == $this->unitQuizDetails->quiz_type ) {
				//$totalQs = count( $this->unitQuizDetails->questions );
				$totalQs = is_countable( $this->unitQuizDetails->questions )? count( $this->unitQuizDetails->questions ):0;
		...
		public function check_quizzes_hasUserCompletedQuiz() {
			...
			if ( isset($this->unitQuizProgress->quiz_paging_status) && 'complete' == $this->unitQuizProgress->quiz_paging_status ) {
				return true;
			}
		 ...
		 * @return String The content to render for the unit.
		 */
		public function render_detailsForCompletebox( $content, $justGetCompletionAndQuizData = false ) {
			$completionBox    = false;
			$extraMessages    = false;
			$quizProgressData = false;
			...
			if ( $justGetCompletionAndQuizData ) {
				...
				// Modify the completion box.
				echo $content . '<div id="wpcw_fe_outer_wrap">' . $completionBox . $quizProgressData  . $extraMessages . '</div>';
			}
		}
		 ...
		 * @return String The content to render for the unit.
		 */
		public function render_detailsForNvigationBox( $content, $justGetCompletionAndQuizData = false ) {
			$completionBox    = false;
			$extraMessages    = false;
			$quizProgressData = false;
		 ...
		 * @return String The content to render for the unit.
		 */
		public function render_detailsForContent( $content, $justGetCompletionAndQuizData = false ) {
			$completionBox    = false;
			$extraMessages    = false;
			$quizProgressData = false;
			...
			if ( $justGetCompletionAndQuizData ) {
				...
				return $content . '<div id="wpcw_fe_outer_wrap">'.$quizProgressData.'</div>';
			}
		}
