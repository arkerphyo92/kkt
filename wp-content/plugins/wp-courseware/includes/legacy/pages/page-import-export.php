		...
		if ( $errornum == 0 ) {
			...
			if ( ( $csvHandle = fopen( $tempfile, "r" ) ) !== false ) {
				...
				foreach ( $assocData as $csvRowKey => $csvRow ) {
					...
					$question_question_text = stripslashes( $csvRow['quiz_question'] );

					// Populate Question
					$question = array(
						'question_type'                    => $csvRow['question_type'],
						'question_question'                => iconv(mb_detect_encoding($question_question_text, mb_detect_order(), true), "UTF-8", $question_question_text),
